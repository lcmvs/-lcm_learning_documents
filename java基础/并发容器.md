...
put的时候，如果数组节点为空，尝试用cas方式非阻塞的将元素放入容器中，当cas失败时，循环，也就是自旋，不为空使用synchronized加锁，但是只锁表节点对象，而不是锁整个对象，增加了并发性。
...
ConcurrentHashMap的get没用任何同步方法，是并发的get，但是因为node是volatile的，所有不会出现线程可见性问题。

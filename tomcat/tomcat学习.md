...
tomcat使用了一个无界队列作为线程池的阻塞队列，但是重写了队列的offer方法，**当其线程池大小小于maximumPoolSize的时候，返回false**。这样线程池的最大线程数就不会失效了，线程池会继续创建线程，直到达到最大线程数，tomcat的阻塞队列是一个无界队列，但是外层使用一个默认10000大小的共享锁LimitLatch来线程最大连接数。
